...
// 用法：环境变量 XHS_KEYWORDS(逗号分隔) / XHS_COOKIE / XHS_LIMIT 传参
//       （不用命令行参数——Windows 下中文参数经 execFile 会 GBK 乱码）
// 优先用 storageState（扫码登录保存的完整浏览器状态），比纯 cookie 稳——小红书不软封锁。
// 移植自 style-atlas（tags/server/src/scripts/xhs-search.mjs），字段/流程一致。
import { chromium } from 'playwright'
import { existsSync } from 'node:fs'
import { join } from 'node:path'

const keywords = (process.env.XHS_KEYWORDS || '').split(',').filter(Boolean)
const cookieStr = process.env.XHS_COOKIE || ''
const limit = Number(process.env.XHS_LIMIT) || 100

const AD_KEYWORDS = ['开课', '摆摊', '发布会', '报名', '课程', '招生', '培训', '兼职', '招聘', '众筹', '预售', '下单', '购买', '淘宝', '拼多多', '闲鱼', '微店', '优惠', '折扣', '抽奖', '转发', '关注我', '求关注', '互粉']
